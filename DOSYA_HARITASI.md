...
Streamlit uygulamasının tamamı; kullanıcının gördüğü HER panel burada render edilir (33 render fonksiyonu). Hesaplar modüllerden import edilir, burada sadece görselleştirilir. Sol sütun (`_render_left_col`, ~%82) tek-hisse detay panellerini, sağ sütun (`_render_right_col`, ~%20) tarama sonuçlarını çizer. Kanıt Terazisi kartı (`_render_kanit_terazisi_card`), Smart Money Hacim paneli (`render_smart_volume_panel`), GENEL ÖZET (`_render_genel_ozet_panel`), Teknik Yol Haritası (`render_roadmap_8_panel`), SİNYAL ÖZETİ matrisi, ICT paneli (`render_ict_deep_panel`) hepsi buradadır. B35 bölümündeki ~4K satırlık AI Prompt if-bloğu (Gemini'ye giden metin) da app.py'de. Master Scan butonu (`💎 TÜM PİYASAYI TARA`) ve ekran tarafındaki ilerleme/kesinti kapısı buradadır; asıl orkestrasyon `master_scan_engine.py` içindedir. ÜYE MODU bloğu (MEMBER_MODE/SHOWCASE_MODE flag'leri, quota/tier gizleme) admin-dışı web sürümü içindir. Endeks dil normalleştiricisi (`_endeks_dil` / `_index_has_tl_ciro`) XU100'de hisse panelini "piyasa geneli" diline çevirir. Bölüm haritası (B1-B37 köşe taşları) ve fonksiyon-satır eşlemesi için → `CLAUDE.md`. **Ne zaman dokun:** ekranda görünen HER şey (renk, metin, panel düzeni, sıralama, AI prompt metni). ⚠️ Yeni HESAP kodu buraya YAZILMAZ — ayrı modüle.

### `master_scan_engine.py` — MASTER SCAN ORKESTRASYON ÇEKİRDEĞİ
Master Scan'in Faz 1/Faz 2 sırasını ve kapanış işlerini Streamlit'ten bağımsız yürütür. `durum` sözlüğü sonuçları taşır, `bildir` kanalı ekran veya log bildirimini alır. **Ne zaman dokun:** yalnız Master Scan'in ekran bağımlılığını kaldıran orkestrasyon işi değişecekse; tarama hesaplarına burada dokunulmaz.

### `master_scan_kos.py` — EKRANSIZ MASTER SCAN KOŞUCUSU
Golden'ın sahte Streamlit yükleme yolunu kullanarak Faz 1 ve Faz 2'yi tek süreçte çalıştırır; gerçek koşuda kapanış kayıtlarını da tamamlar, `--kuru` modunda patron.db ve tamamlanma kaydına yazmaz. **Ne zaman dokun:** ekran dışı koşu, log veya çıkış kodu sözleşmesi değişecekse.

### `master_scan_kiyas.py` — MASTER SCAN KIYAS HAKEMİ
İki tarihli turun `scan_runs` ve `scan_signals` kayıtlarını tarama tipi, satır sayısı, sembol kümesi ve üç sayısal alan için sıfır toleransla karşılaştırır; Markdown ve JSON raporu üretir. **Ne zaman dokun:** kıyas ölçütü bilinçli olarak değiştirilecekse; fark çıktıktan sonra eşik gevşetilmez.
...
### `magic_ribbon_core.py` + `magic_ribbon_session_data.py` — MAGIC RIBBON BIST SEANS-MUMU GÖZLEM MOTORU
Kullanıcının paylaştığı Magic Ribbon Fast/Slow hizalanmasını ayrı TradingView 5 dakikalık kasasından hesaplar. Her tam işlem günü 09:55–14:00 ve 14:00–18:10 olmak üzere iki seans mumu kurulur; yalnız işlem oluşmayan 18:00 aralığı boş kalabilir, bunun dışındaki her eksikte gün reddedilir. `veriler_4s/`, Yahoo hattı ve diğer 4S taramalarına dokunmaz. `_bist100.json` evreni kesin olarak BIST100'e sınırlar; sonuç skora, teraziye ve AI prompt'una bağlanmaz. `magic_ribbon_refresh.py` + `run_magic_ribbon.sh` kapanıştan sonra tek tek, düşük tempoyla yeniler ve seans kasasını VPS'e yollar. **Ne zaman dokun:** Magic Ribbon formülü, seans sınırları veya yalnız bu gözlem hattının çalışma biçimi değişecekse.
...
`golden_record.json` (emniyet referansı) · `backtest_results.json` · `genel_ozet_verdict_backtest.json` (Smart Money verdict karnesi) · `panel_verdict_backtest.json` · `rsi_kova_backtest.json` · `sert_gun_backtest.json` · `tavan_weights_onerilen.json` · `magic_ribbon_4s_backtest.json` · `magic_ribbon_session_backtest.json` (yalnız son yaklaşık 55 seansın kısa tarihçesi). **Rejim değişince** ilgili backtest yeniden koşulur.
...
- `veriler/*.parquet` — her hissenin günlük OHLCV cache'i (SYMBOL.IS_1d.parquet). `veriler_4s/*.parquet` — eski 4S ölçümleri için 4 saatlik kasa. `veriler_magic_ribbon_5m/*.parquet` ve `veriler_magic_ribbon_seans/*.parquet` — yalnız Magic Ribbon'un TradingView 5 dakikalık ham verisi ve iki seans mumundan oluşan ayrı kasası. `_bist100.json` — Magic Ribbon gözlem evreni.
