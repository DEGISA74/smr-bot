...
### Çözülen Sorunlar (1 Haz 2026 — Oturum 11: B5 HVN/LVN + UX overhaul + AI prompt zenginleştirme)
**app.py-only oturum.** Hedef: kurumsal seviye sinyal kalitesi + "to the point" abone UX.

**🎯 Analiz/Sinyal:**
- ✅ **B5 — HVN/LVN expansion** (`calculate_price_action_dna` ~20620): Volume Profile lookback **20→60g** (kurumsal birikim seviyesi); 3 yeni AI veri alanı (`hvn_en_yakin`, `fiyat_lvn_icinde`, `fiyat_poc_konumu`); prompt rehberi 6 satır (±%2 yakınlık → mıknatıs, LVN içinde → hızlı geçer, POC üstü kırılım denemesi).
- ✅ **VP Şekil tespiti** (yeni — Akümülasyon/Dağıtım/Denge): POC'un Value Area içindeki konumuna göre. AI prompt'a `smart_money.vp_sekil` + 4 satır rehber. Panel'de "Son 20G Range" satırına şekil chip'i.
- ✅ **CP Yanıltıcı tespiti** (CP × 5g net %): Gap-grind koruması — "Mum %85'te AMA fiyat 5g net -3.5%" durumunda eski "alıcı baskısı" etiketi yerine "Yanıltıcı ⚠" + pulse uyarısı. `_render_genel_ozet_panel` Mum satırı tamamen refactor (10→7 senaryo, başlık `"Mum (Kapanış Konumu)" → "Kapanış, o günkü mumun neresinde"`). AI prompt'a `smart_money.mum_kapanis_durumu` + 4 satır rehber.
- ✅ **B4 altyapı** (`scan_signals` tablosu): `obv_status TEXT` kolonu eklendi (idempotent ALTER). `log_scan_signal` df'ten OBV_Status okur. 2 hafta veri birikince ŞÜPHELİ etiketinin hit_rate kalibrasyonu için altyapı hazır.

**🎨 UX/Render (UI-only):**
- ✅ **GENEL ÖZET sade Türkçe** — "RSI 5g'de −8.9 — düşüş hızlanıyor" → "RSI son 5 günde −8.9 düştü". "Orta vade (2–3 ay) · SMA50 altında" → "50 günlük ortalamanın 24 gündür altında". Range/CP cümleleri "5 gün önce fiyat 20 günlük aralığının %X'sindeydi, şimdi %Y'sinde" format'ına dönüştürüldü. "Orta bölge" → "orta bölgede (ne dipte ne tepede)".
- ✅ **Dinamik MA tablosu** (~22720): Eski 7-MA statik şerit silindi. Yeni: yatay tek satır, MA'lar değere göre azalan sıralı (sola en yüksek → sağa en düşük), fiyat satırı vurgulu gradient, üstündekiler 🔴 Direnç, altındakiler 🟢 Destek. Kısa vade mavi (EMA5/8/13), orta vade mor (SMA50/100/200, EMA144). Padding/gap/line-height optimize.
- ✅ **VP konum + şekil chip'leri** (Son 20G Range satırı): Bölgeler Türkçeleşti — `Premium → Pahalı Bölge`, `Discount → Ucuz Bölge`, `Value Area → Değer Bölgesi`. Yanına ikinci chip: `Akümülasyon / Dağıtım / Denge`. POC ham sayısı sol panel'den **silindi** (kullanıcı isteği — hesapta+AI prompt'ta korundu).
- ✅ **Sağ üst metrik kart refactor** (~23938): GÜVEN satırı + alt chip **tamamen kaldırıldı**. YAPI tek satırdan **3-katmanlı sub-block**'a çevrildi: `Kısa (1h) / Orta (1-2 ay) / Makro (6-9 ay)`. Kısa = son 6 mum HH/HL/LH/LL; Orta = ICT bias; Makro = `detect_market_regime` Wyckoff faz. Endeks/kripto/emtia'da RS GÜCÜ "—" ise satır gizlenir; MOMENTUM sağ kolondan alt şerite taşınır (RSI ile yan yana, YAPI'ya nefes alanı).

**🔤 Terminoloji tutarlılığı:**
- ✅ **CMF / Chaikin Money Flow → "Para Akışı"** — Tüm render string'lerinde + AI prompt rehberinde. Etiketler: `(CMF Çelişkili)` → `(Para Akışı Çelişkili)`, `(OBV-CMF Çelişkisi)` → `(OBV-Para Akışı Çelişkisi)`, `(OBV güçlü, CMF zayıf)` → `(OBV güçlü, Para Akışı zayıf)`, `| CMF:` → `| Para Akışı:`. Code yorumları (#) dokunulmadı. Tile 6 regex backward-compat (`(?:Para Akışı|CMF):`).

**⚠️ Kalıcı Yasaklar (`memory/feedback_sektor_rejim_yasak.md`):**
- ❌ Hisse analizine **sektör endeksi** (XBANK, XGIDA, XHOLD vb.) veya **piyasa rejimi** context'i ASLA eklenmez. `detect_market_regime` UI'da (sağ kart Makro) kullanılır ama scanner filtresine **ve** AI prompt'a global rejim context'i olarak bağlanmaz. Kullanıcı 1 Haz 2026 oturumunda kesin reddetti.

**⏳ Beklemede (kullanıcı kararı):**
- B1 (peak_day AI prompt'a) — `signal_returns` 20G verisi olgunlaşınca yeniden gündeme gelecek. Şu an erken.
- B4 ŞÜPHELİ kalibrasyon — 2 hafta veri birikince eşik kalibrasyonu.
- **Hayalet bar Plan B** — `_strip_holiday_bars` şu an sadece TRAILING V=0 barları siliyor. Plan B: ortadaki (gömülü) V=0 barları da silmek → tüm tüketiciler (scanner'lar, ICT, Smart Money, backtest) otomatik faydalanır. Plan A (1 Haz 2026) AI prompt + GENEL ÖZET + Sentiment grafiği giriş noktalarına lokal filter koydu — en kritik 2 tüketici temiz. B planı 2-3 hafta backtest verisi olgunlaşınca, hayalet bar etkisi ölçülüp karar verilecek (tarama sonuçları değişebilir: 5G/20G ortalamalar bar pozisyonu bazlı, takvim bazlı değil).

### Çözülen Sorunlar (1-2 Haz 2026 — Oturum 12: GENEL ÖZET Kurumsal Görsel Overhaul + CMF Dual-Window)
**app.py-only oturum.** Hedef: GENEL ÖZET panelini "text-heavy" yerine "shape-driven" yap (Bloomberg/data viz benzeri); CMF'ye OBV mantığıyla dual-window ekle.

**🎨 GENEL ÖZET Görsel Overhaul (sol panel — `_render_genel_ozet_panel`):**
- ✅ **Verdict band — 3 görsel katman:**
  - **5-mum mini şerit** (SVG, 110×80px): son 5 OHLC mumu yan yana, body+fitil renkli. "Bugünkü kapanış mumun %X'inde" uzun açıklamayı görselleştirir.
  - **Arrow stack** (dikey kompakt): 4 sinyal (HACİM/OBV/YAPI/RSI) sol kolonda, etiket+ok yan yana, renk kodlu. Mumlarla yan yana.
  - **Multi-TF Traffic Lights** (dikey, sağ kolon): 5g/20g/50g/200g — yeşil/kırmızı/gri ışıklar. 1g atıldı (gürültü). Verdict band'ın sağ tarafına yerleşti.
- ✅ **Section başlıklarına Heatmap Dots** (`_section_dots`): Yapı & Konum (1 dot _sig_yapi), Yön (2 dots ana trend + kısa vade), Para akışı (2 dots _sig_hacim + _sig_obv), Momentum (1 dot _sig_rsi). Tek bakışta bölüm yönü özeti. `_gs_section` `badge_html` parametresi wrapper'a sarıldı (margin-left:auto auto'a).
- ✅ **LONG skor hisselerde de gerçek değerle çalışıyor** — pre-existing bug fix: `evaluate_erken_radar(_ticker, df, _bench_df)` çağrısında `df` undefined idi (NameError sessizce yutuluyordu, _lr_score=None kalıyordu). Düzeltildi: `df` → `_gs_df`. Önceden "—" gözüküyordu, artık gerçek Erken Radar overall_quality skoru.

**🧭 PARA AKIŞI — Force Compass Tasarımı:**
- ✅ **Pusula (110×100 SVG):** 4 yön (BOĞA/AYI/SAT/AL Türkçe). 3 distinct shape — vade hiyerarşisi görsel:
  - **▲/▼ üçgen = OBV** (Y ekseni, uzun vade) — yön yön'ü gösterir
  - **◆ elmas = Para Akışı / CMF** (NE-SW diagonal, orta vade)
  - **■ kare = Δ 5g** (NW-SE diagonal, kısa vade)
- ✅ **Görünürlük:** `_safe_pos` ile minimum 0.35 offset — nötr sinyaller de görünür konumda. Renkler dinamik (yeşil/kırmızı/gri). Bileşke ok arkada (opacity 0.55).
- ✅ **Legend (3-sinyal listesi):** Compass'taki **AYNI ŞEKİL + RENK** ikonu etiketin önünde (`_mini_obv_icon/_mini_cmf_icon/_mini_d5_icon`). Eşleme net. Padding-left 38px (compass'tan uzak).
- ✅ **3 cümle altta** (Hacim + OBV + Para Akışı): Italic, `_gs_expl_col` ile panel açıklama stiliyle aynı. İkonlar normal. `_gs_line` çerçeve içinde.
- ✅ **VSA detection kaldırıldı** (Hacim cümlesi sade RVOL + 5g delta'ya indirildi).

**🌊 CMF Dual-Window (Phase 1+2):**
- ✅ **Panel:** `compute_cmf(df, period=5)` yeni eklendi, 20g ile birlikte 7 state machine:
  - `strong_pos` (5g+ ve 20g+) → ✓ "5g ve 20g birlikte pozitif"
  - `strong_neg` (5g- ve 20g-) → ⚠ "5g ve 20g birlikte negatif"
  - `turning_up` (5g+ AMA 20g-) → ↗ "Bar içi toparlanma erken sinyali"
  - `turning_down` (5g- AMA 20g+) → ↘ "Bar içi alıcı yorgun, erken uyarı"
  - `pos`/`neg`/`neutral` (sadece 20g aktif veya hiçbiri)
  - State driven `_cmf_strength` (turning durumları ±0.3 zayıf konum), legend etiketi ve cümle.
- ✅ **AI prompt:** `YAML.obv_cmf.cmf_dual_window` yeni alan + 6 satır yorumlama rehberi. AI artık "5g pozitif, 20g hâlâ negatif" gibi nüansları yorumlayabilir.
- ⏳ **Phase 3 beklemede** (MEMORY.md): `get_obv_divergence_status` + `calculate_price_action_dna` + `process_single_accumulation` + `render_smart_volume_panel` Tile 6. Backtest gözlemi sonrası karar.

**💎 Sağ Üst Kart (FIYAT) Refactor:**
- ✅ **GÜVEN tamamen silindi** (üst metrik + alt chip).
- ✅ **YAPI 3-katmanlı sub-block:**
  - **Kısa (1h):** Son 6 mum HH/HL/LH/LL (Sağlam ↑ / Bozuk ↓ / Megafon ⚠ / Üçgen / Karışık)
  - **Orta (1-2 ay):** ICT bias (BULLISH ↑ / BEARISH ↓ / NÖTR)
  - **Makro (6-9 ay):** `detect_market_regime` Wyckoff faz (Birikim/Yükseliş/Dağıtım/Düşüş/Geçiş)
- ✅ **BIST (3 seksiyon):** FIYAT|YAPI top / RS GÜCÜ|MOMENTUM middle / RSI|HACİM|BETA bottom.
- ✅ **Endeks/Kripto/Emtia (2 seksiyon):** Middle strip yok; MOMENTUM alt şerite RSI ile yan yana. RS GÜCÜ "—" gizleniyor.

**📊 Dinamik MA Tablosu:**
- ✅ Eski 7-MA statik şerit silindi. Yeni: yatay tek satır, MA'lar değere göre azalan sıralı (sola en yüksek → sağa en düşük). Fiyat satırı vurgulu gradient (mavi-mor), üstündekiler 🔴 Direnç, altındakiler 🟢 Destek. Kısa vade mavi (EMA5/8/13), orta vade mor (SMA50/100/200, EMA144). Padding 3px, gap 0, line-height 1.05 (kompakt).

**🎯 RSI Bullseye:**
- ✅ Eski yatay gauge → concentric zone disc (28×28 SVG). Dış kırmızı halka (extreme), iç yeşil ring (normal 30-70), yatay merkez referans (RSI=50). Marker dot dikey eksende: RSI>50 yukarı, <50 aşağı; renk extremity'ye göre yeşil/sarı/kırmızı.

**📝 Text Refactor (GENEL ÖZET):**
- ✅ Range/CP/Momentum/Ana trend cümleleri sade Türkçe:
  - "RSI 5g'de -8.9 — düşüş hızlanıyor" → "RSI son 5 günde -8.9 düştü"
  - "Orta vade (2–3 ay) · SMA50 altında · 24 gün · Köklü düşüş · baskılı" → "50 günlük ortalamanın 24 gündür altında · Köklü düşüş · baskılı"
  - "5 gün önce fiyat son 20 günlük aralığın %X'sindeydi, şimdi %Y'sinde" formatı korundu, "orta bölge" → "orta bölgede (ne dipte ne tepede)"
- ✅ **"Yeni yapı oluşuyor" 4 detaylı senaryo** (eskiden 3, hepsi aynı belirsiz cümle):
  - HH only: "Son tepe öncekinden yüksek (HH) — alıcılar yeni zirve denedi, henüz HL teyidi yok"
  - HL only: "Son dip öncekinden yüksek (HL) — satıcılar zayıfladı, henüz HH yapılmadı"
  - LH only: "Son tepe öncekinden düşük (LH) — alıcılar yoruldu, henüz LL kırılmadı"
  - LL only: "Son dip öncekinden düşük (LL) — satıcılar baskın, henüz LH teyidi yok"

**🔧 Bug Fix — Hayalet Bar Filter v2:**
- ✅ Önceki filtre `Volume <= 0` → endeks legitimate V=0 günlerini de siliyordu (XU100 1 Haziran sorunu). Yeni: **V=0 VE O==H==L==C (flat)** çift koşul. `_strip_holiday_bars` mantığı klonlanmış, 3 yere (synthetic_sentiment chart + GENEL ÖZET `_gs_df` + AI prompt `df_hist`) uygulanmış. Endeks/kripto data bütünlüğü korundu.

**🚀 Erken Radar Panel:**
- ✅ **5G önce skor** italic gri satır eklendi (`evaluate_erken_radar` 5g öncesi df ile yeniden çağrılır). Veri yetersizse satır gizlenir. Skor evrim göstergesi.

**📌 Diğer:**
- ✅ "CMF / Chaikin Money Flow" → "Para Akışı" tüm render + AI prompt rehberi (zaten önceki oturumdan, devam).
- ✅ AI prompt'a VP Şekil (Akümülasyon/Dağıtım/Denge) + CP Yanıltıcı (CP × 5g net %) rehberi (önceki commit ile birlikte).
