...
# Gövde taşma süzgeci sabitleri + yardımcıları TEK KAYNAK: formasyon_v2.py
# (_v2.WEDGE_OVERSHOOT_*, _v2._body_overshoot_stats, _v2._overshoot_penalty).
# V3 geniş dedektörü de aynı ayarı ve aynı ölçümü kullanır.
...
def _detect_wide_triangles(
    ...
    for top, top_points, top_score in top_lines:
        for bottom, bottom_points, bottom_score in bottom_lines:
            ...
            # Kapanışı sınır çizgisi dışına taşan mumları cezalandırır (fitil
            # hariç); 4+ ardışık dışarıda kapanış adayı reddeder. Ölçüm + ceza
            # tek kaynak: formasyon_v2 yardımcıları (v2 ile birebir aynı).
            _ov_end = min(int(line_end_bar), n - 1)
            body_overshoot_count, body_overshoot_ratio, body_overshoot_run = (
                _v2._body_overshoot_stats(
                    close, start, _ov_end, top.at, bottom.at, price_ref
                )
            )
            if body_overshoot_run >= _v2.WEDGE_OVERSHOOT_MAX_RUN:
                continue
            overshoot_penalty = _v2._overshoot_penalty(
                body_overshoot_count, body_overshoot_ratio
            )
