...
def build_feature_frame(df: pd.DataFrame, market: pd.DataFrame) -> pd.DataFrame:
    ...
    # B11 rozeti yalnızca çapraz teyittir; model olasılığına/puanına katılmaz.
    b11_high_60 = high.rolling(60, min_periods=60).max()
    b11_low_60 = low.rolling(60, min_periods=60).min()
    out["b11_pos_60"] = _safe_div(close - b11_low_60, b11_high_60 - b11_low_60).mul(100.0)
    b11_width = _safe_div(
        close.rolling(20, min_periods=20).std(),
        close.rolling(20, min_periods=20).mean(),
    )
    b11_width_average = b11_width.rolling(20, min_periods=20).mean()
    out["b11_teyidi"] = (
        (out["b11_pos_60"] > 80.0)
        & (b11_width < b11_width_average * 0.85)
        & (out["relative_ret_20g"] > 0.0)
    )
