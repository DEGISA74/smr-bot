...
def build_teknik_ozet(ticker: str, df: "pd.DataFrame | None" = None, ict: dict = None, info: dict = None) -> str:
    ...
    try:
        ...
        try:
            if n >= 6:
                _hs_o = h.values; _ls_o = l.values
                _h1o, _h2o, _h3o = _hs_o[-1], _hs_o[-2], _hs_o[-3]
                _l1o, _l2o, _l3o = _ls_o[-1], _ls_o[-2], _ls_o[-3]
                _avg_o = max(1e-9, (max(_hs_o[-6:]) - min(_ls_o[-6:])) / 5)
                _tol_o = _avg_o * 0.15
                _hh_o = _h1o > _h2o > _h3o; _hl_o = _l1o > _l2o > _l3o
                _lh_o = _h1o < _h2o < _h3o; _ll_o = _l1o < _l2o < _l3o
                if _hh_o and _hl_o:                                                  _yapi_tip = "hh_hl"
                elif _lh_o and _ll_o:                                                _yapi_tip = "lh_ll"
                elif _hh_o and _ll_o:                                                _yapi_tip = "megafon"
                elif _lh_o and _hl_o:                                                _yapi_tip = "ucgen"
                elif abs(_h1o-_h3o) < _tol_o and abs(_l1o-_l3o) < _tol_o:            _yapi_tip = "yatay"
                elif _hh_o or _hl_o:                                                 _yapi_tip = "yari_yukari"
                elif _lh_o or _ll_o:                                                 _yapi_tip = "yari_asagi"
        except Exception:
            pass
...
2) RSI overbought TEK BAŞINA: × "RSI 75 → satış geliyor" · ✓ "RSI 75 — trendde 70+ kalabilir, OBV çelişkisi yoksa düzeltme zorunluluğu yok"
3) Z-Score TEK BAŞINA: × "Z>+2.5 → çöküş" · ✓ "Z-Score ivme ölçüsü; izleyen stop noktası"
4) Pahalı/Ucuz YARGISI yüzdelik uzaklık üzerinden: × "%5 yukarıda → pahalı bölge" · ✓ "fiyat X seviyesi üzerinde" (yargı yok, seviye var)
5) Mean reversion bağımsız kanıt OLMADAN: × "geri gelmeli / ortalamaya döner / kaçınılmaz düzeltme" · ✓ sadece a/b/c/d koşullarından biri varsa

YASAKLI KALIP AİLELERİ (fikri yakala — hepsi yasak):
× "düzeltme yakın / pahalı / aşırı uzak / geri gelmeli / ortalamaya döner / sürdürülemez / adil değerden saptı / RSI 70+ satış / Z>+2 çöküş yakın / parabolik bitti / kâr satışı yakın / yapısal zayıflık (uzaklık nedeniyle)"
...
× "VWAP'ın %X altında/üstünde işlem görmesi yapısal zayıflıktır/güçtür" (uzaklık → yargı yasak)
