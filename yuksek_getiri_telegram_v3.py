#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Gelişmiş Yüksek Getiri Motoru V3 Telegram göndericisi.

Eski tavan/yüksek getiri göndericisinden tamamen bağımsızdır. Her çalışmada:
1) Bir önceki V3 listesinin sonraki seans gün içi zirve karnesini,
2) Güncel V3 ilk 5 aday listesini gönderir (backtest: top-5 en yüksek güven).
"""
...
DISPLAY_REASON = {
    ...
    "para_akisi": "para akışı",
    "piyasa": "piyasa rejimi",
    "v1_kategori": "kalıp (V1)",
}
...
def _previous_message(results: list[dict], evaluation_date: pd.Timestamp | None) -> str | None:
    ...
    lines = [
        "📊 DÜNKÜ GELİŞMİŞ YÜKSEK GETİRİ MOTORU V3 — NE YAPTI?",
    ...
    for number, row in enumerate(results):
        ...
        lines.append(
            f"{marker} {row['ticker']:<6} {row['return_pct']:+.1f}%   "
...
def _today_message(candidates: pd.DataFrame, metadata: dict, model: dict) -> str:
    ...
    lines = [
        f"🚀 GELİŞMİŞ YÜKSEK GETİRİ MOTORU V3 — {today}",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Gelişmiş Yüksek Getiri Motoru V3 Telegram göndericisi")
    parser.add_argument("--test", action="store_true", help="Yalnızca yöneticiye önizleme; DB kaydı YAPMAZ")
    parser.add_argument("--broadcast", action="store_true", help="SMR Elite grubuna yayınlar (varsayılan: gölge=admin, DB kaydeder)")
    ...
    parser.add_argument("--top", type=int, default=5, help="Gösterilecek aday sayısı")
    ...
    parser.add_argument("--state", default=str(BASE / "yuksek_getiri_v3_state.json"))
    parser.add_argument("--db", default=str(BASE / "patron3.db"))
    return parser
...
def main() -> int:
    ...
    if candidates.empty:
        raise DataUnavailable("V3 uygun aday havuzu üretilemedi; günlük veri eksik veya güncel değil.")
    ...
    if not args.test and not args.dry_run and state and state.get("as_of") == target_date:
        try:
            ...
            mark_run_published(db_path, "v3", target_date, len(state.get("list") or []))
        except Exception as exc:
            print("Patron DB V2 sicil onarımı başarısız:", exc)
            return 1
        print(f"{target_date} kapanışı için V3 listesi daha önce gönderilmiş; tekrar gönderim yapılmadı.")
        return 0
    ...
    if report:
        ...
        print("(Değerlendirilebilir önceki V3 listesi yok; karne atlandı.)")
    print(today_message)
    ...
    targets = BROADCAST if args.broadcast else (ADMIN_ID,)
    if report:
        _send_to_targets(targets, report, "karne gönderimi")
    today_sent = _send_to_targets(targets, today_message, "V3 gönderimi")

    if not args.test and today_sent:
        _save_state(state_path, target_date, candidates, metadata, model)
        try:
            mark_run_published(db_path, "v3", target_date, len(candidates))
        except Exception as exc:
            print("V3 listesi gönderildi fakat Patron DB yayın işareti yazılamadı:", exc)
            return 1
        print("V3 günlük kayıt dosyası güncellendi:", state_path)
    elif not args.test:
        print("V3 listesi tüm hedeflere ulaşmadığı için günlük kayıt dosyası değiştirilmedi.")
        return 1
    return 0
...
if __name__ == "__main__":
    raise SystemExit(guarded_main(
        engine="v3_yuksek_getiri",
        label="GELİŞMİŞ YÜKSEK GETİRİ MOTORU V3",
        main_func=main,
